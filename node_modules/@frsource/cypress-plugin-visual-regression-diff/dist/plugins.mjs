...
const parsePathPartVariables = (specPath, pathPart, i) => {
...
const generateScreenshotPath = ({
  ...
  const screenshotPath = path.join(...imagesPath.split("/").map(parsePathPartVariables.bind(void 0, specPath)), sanitizeFilename(titleFromOptions));
...
const addPNGMetadata = png => addMetadata(png, METADATA_KEY, DIFF_IMAGES_VERSION
/* c8 ignore next */
);
const getPNGMetadata = png => getMetadata(png, METADATA_KEY
/* c8 ignore next */
);
const isImageCurrentVersion = png => getPNGMetadata(png) === DIFF_IMAGES_VERSION;
const isImageGeneratedByPlugin = png => !!getPNGMetadata(png
/* c8 ignore next */
);
const writePNG = (name, png) => fs.writeFileSync(name, addPNGMetadata(png instanceof PNG ? PNG.sync.write(png) : png));
...
const cleanupUnused = rootPath => {
  glob.sync("**/*.png", {
    ...
    if (!wasScreenshotUsed(pngPath) && isImageGeneratedByPlugin(fs.readFileSync(absolutePath))) {
...
const unlinkSyncSafe = path => fs.existsSync(path) && fs.unlinkSync(path);

const moveSyncSafe = (pathFrom, pathTo) => fs.existsSync(pathFrom) && moveFile.sync(pathFrom, pathTo);

const getScreenshotPathInfoTask = cfg => {
  const screenshotPath = generateScreenshotPath(cfg);
  return {
    screenshotPath,
    title: path.basename(screenshotPath, ".png")
  };
};
...
const compareImagesTask = async cfg => {
  ...
  if (fs.existsSync(cfg.imgOld) && !cfg.updateImages) {
    ...
    const diffPixels = pixelmatch(imgNew.data, imgOld.data, diff.data, width, height, diffConfig);
    ...
    if (cfg.createMissingImages) {
      ...
      error = true;
      messages.unshift(`Baseline image is missing at path: "${cfg.imgOld}". Provide a baseline image or enable "createMissingImages" option in plugin configuration.`);
    }
  }
...
const initAfterScreenshotHook = config => details => {
  ...
  const screenshotPath = details.name.substring(IMAGE_SNAPSHOT_PREFIX.length + path.sep.length);
  ...
  return (async () => {
    ...
    await promises.rm(path.join(screenshotsFolder, IMAGE_SNAPSHOT_PREFIX), {
      recursive: true,
      force: true
    });
    return {
